...
void Vision_Move(void)
{
	...
	{
		//检测数据是否正确
		if(isnan(VisionValue.vision_left_pitch_value.value)||isinf(VisionValue.vision_left_pitch_value.value))
		{
			err_sum.vision_pitch++;
			return ;
		}
		if(isnan(VisionValue.vision_left_yaw_value.value)||isinf(VisionValue.vision_left_yaw_value.value))
		{
			err_sum.vision_yaw++;
			return;
		}
		...
		{
			yaw_value = VisionValue.vision_left_yaw_value.value - YAW_Vision;
			pitch_value = VisionValue.vision_left_pitch_value.value - PITCH_Vision;
			//自瞄
		  if((fabs(yaw_value) < ((0.135f /(2.f * VisionValue.vision_Left_Gimbal_distance.value)) * 0.7f) && fabs(pitch_value) < ((0.135f /(2.f * VisionValue.vision_Left_Gimbal_distance.value)) * 0.7f) ) && VisionValue.find_bool == '1') 
			{
			...
			{
				Gimbal_Data.IMUData.yaw_target = ((VisionValue.vision_left_yaw_value.value + PI - 1.04694188f) / PI) * 4096;
				Gimbal_Data.IMUData.pitch_target = VisionValue.vision_left_pitch_value.value;
			}
			else
			{
...
void Remote_Move()
{
	...
	//			Scan_TIME = (Scan_TIME + 1) % 1000;
				...
				else if(Motor_6020[0].angle < YAW_ANGLE_MIN && Motor_6020[0].angle > 3480)
				{
					Gimbal_Data.IMUData.yaw_target -= Yaw_Move_Speed;
