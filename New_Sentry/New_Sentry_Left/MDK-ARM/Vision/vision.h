...
//云台控制内容
	...
	union
	{
		uint8_t buff[4];
		float value;
	}vision_left_yaw_value;
	union
	{
		uint8_t buff[4];
		float value;
	}vision_left_pitch_value;
	union
	{
		uint8_t buff[4];
		float value;
	}vision_Left_Gimbal_distance;
...
typedef struct{
	...
	union
	{
		...
		float value_offset;
	}vision_left_yaw_gyro;
	union
	{
		uint8_t buff[4];
		float value_offset;
	}vision_left_pitch_gyro;
...
extern void vision_call_back_handler_Left_Control(uint8_t *buff);
