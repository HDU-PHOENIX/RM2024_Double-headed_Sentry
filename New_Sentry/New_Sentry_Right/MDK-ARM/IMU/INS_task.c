...
float *IMU_kf_result;
float yaw_add_one_tick=-0.0000656595769f;
float yaw_offset;
				//消零飘
int pre_tick = 0;
int tick;
...
//				imu_cali_slove(bmi088_real_data.gyro,accel_fliter_3,ist8310_real_data.mag,&bmi088_real_data,&ist8310_real_data);
        ...
        IMU_angle[0] = 180.0f*INS_angle[0]/PI ; //yaw   yaw_offset
        IMU_angle[1] = 180.0f*INS_angle[1]/PI; //pitch
        IMU_angle[2] = 180.0f*INS_angle[2]/PI; //roll
				pre_tick = tick; 
				
				float res = INS_angle[0];
				add_angle_yaw += (0.00020)/360*2*_PI;
				if(add_angle_yaw > _2PI) add_angle_yaw -= _2PI;
				res -= add_angle_yaw;
				__normalangle(&res,-_PI,_PI);
				if(add_angle_yaw+INS_angle[0] > PI)  
				Last_ol[0] = Gimbal_Data.IMUData.ol[0];
				Last_ol[1] = Gimbal_Data.IMUData.ol[1];
				Last_ol[2] = Gimbal_Data.IMUData.ol[2];
				Gimbal_Data.IMUData.ol[0] = res - yaw_offset / 180;           //yaw
				Gimbal_Data.IMUData.ol[1] = INS_angle[2];           //pitch
				Gimbal_Data.IMUData.ol[2] = INS_angle[1];           //roll
				Gimbal_Data.IMUData.accel[0] = accel_fliter_3[0];
				Gimbal_Data.IMUData.accel[1] = accel_fliter_3[1];
				Gimbal_Data.IMUData.accel[2] = accel_fliter_3[2];
				for(uint8_t i = 0; i < 4; i++)
				{
					Gimbal_Data.IMUData.quat[i] = INS_quat[i];             
				}				
				for(uint8_t i = 0; i < 3; i++)
				{
					Gimbal_Data.IMUData.gyro[i] = bmi088_real_data.gyro[i];    //0 roll    1 pitch     2 yaw
					Gimbal_Data.IMUData.ol_accel[i] = (Gimbal_Data.IMUData.ol[i] - Last_ol[i]) * 1000;
				}	

//------------------------------更新底盘陀螺仪数据-----------------------------
//				BalanceCar.Chass_IMU.ol[0]=INS_angle[0];           //yaw
//				BalanceCar.Chass_IMU.ol[1]=INS_angle[1];           //pitch
//				BalanceCar.Chass_IMU.ol[2]=INS_angle[2];           //roll
//				
//				BalanceCar.Chass_IMU.accel[0]=accel_fliter_3[0];
//				BalanceCar.Chass_IMU.accel[1]=accel_fliter_3[1];
//				BalanceCar.Chass_IMU.accel[2]=accel_fliter_3[2];
//				
//				for(uint8_t i = 0; i < 4; i++)
//				{
//					BalanceCar.Chass_IMU.quat[i] = INS_quat[i];             
//				}				
//				for(uint8_t i = 0; i < 3; i++)
//				{
//					BalanceCar.Chass_IMU.gyro[i] = bmi088_real_data.gyro[i];    //0 roll    1 pitch     2 yaw
//				}					
//				BalanceCar.Chass_IMU.temp = bmi088_real_data.temp;
//				float res = INS_angle[0];
//				add_angle_yaw += (0.00020)/360*2*_PI;
//				if(add_angle_yaw > _2PI) add_angle_yaw -= _2PI;
//				res -= add_angle_yaw;
//				__normalangle(&res,-_PI,_PI);
//				if(add_angle_yaw+INS_angle[0] > PI)  
//				Last_ol[0] = GimbleData.IMUData.ol[0];
//				Last_ol[1] = GimbleData.IMUData.ol[1];
//				Last_ol[2] = GimbleData.IMUData.ol[2];
//				GimbleData.IMUData.ol[0] = res;           //yaw
//				GimbleData.IMUData.ol[1] = INS_angle[2];           //pitch
//				GimbleData.IMUData.ol[2] = INS_angle[1];           //roll
//				GimbleData.IMUData.accel[0] = accel_fliter_3[0];
//				GimbleData.IMUData.accel[1] = accel_fliter_3[1];
//				GimbleData.IMUData.accel[2] = accel_fliter_3[2];
//				for(uint8_t i = 0; i < 4; i++)
//				{
//					GimbleData.IMUData.quat[i] = INS_quat[i];             
//				}				
//				for(uint8_t i = 0; i < 3; i++)
//				{
//					GimbleData.IMUData.gyro[i] = bmi088_real_data.gyro[i];    //0 roll    1 pitch     2 yaw
//					GimbleData.IMUData.ol_accel[i] = (GimbleData.IMUData.ol[i] - Last_ol[i]) * 1000;
//				}	
//-----------------------------------------------------------------------------------
//				IMU_angle[0]=180.0f*get_yaw_ist8310(&ist8310_real_data,INS_angle[1],INS_angle[2])/PI;		//通过磁力计读yaw值，解决了零漂问题，但容易受到外界磁场干扰。磁力计还没校准。
