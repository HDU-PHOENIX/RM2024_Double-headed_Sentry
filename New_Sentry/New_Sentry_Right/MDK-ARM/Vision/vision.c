...
void vision_call_back_handler_Right_Control(uint8_t *buff)//对视觉接收数据进行处理
{
    ...
    if( buff[0]=='s' && buff[31]=='e' )
    {
        if(buff[1] == 0xA0) // 云台控制
        {
            ...
            vision_missing_time = 0;  //清零失联时间
						...
						memcpy(&(VisionValue.vision_right_yaw_value.buff[0]),&buff[3],4);  //yaw 偏移量
						memcpy(&(VisionValue.vision_right_pitch_value.buff[0]),&buff[7],4);  // pitch 偏移量
						memcpy(&(VisionValue.vision_Right_Gimbal_distance.buff[0]),&buff[11],4);  //装甲板距离
					  memcpy(&(VisionValue.real_heat.buff[0]),&buff[15],2);//枪管实时热量
	
        }
    }
		if( (VisionValue.vision_right_yaw_value.value > 100 ) || (VisionValue.vision_right_pitch_value.value > 100 ) )
    {
        VisionValue.vision_right_yaw_value.value = 0 ;
        VisionValue.vision_right_pitch_value.value = 0 ;
    }        
}
...
void vision_transmit_package_AutoaimFeedback (uint8_t *buff)
{
		...
		YAW_Vision = ((Motor_6020[0].angle - 4096.f) / 4096.f ) * PI + 0.99401953359375f;
	  PITCH_Vision = -(Motor_4310.angle - 1.2853f);
