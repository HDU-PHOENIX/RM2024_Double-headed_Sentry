...
extern uint8_t Tx_buff_Line[4][200];
...
typedef __packed struct
{
    Chasis_Recive  chasis_data;																				//帧头
    Gimbal_State   gimbal_state;																										//脕E丒
    uint8_t        Pitch_offset; 	//数据段头结构
    uint8_t        Reset;																				//操作数据类型
	  uint8_t        Shoot_Flag_Open;
	  uint8_t        Shoot_Flag_Close;
	  uint8_t        Shoot_Flag_Slow;
	  uint8_t        reserved[12];
}Student_Data;
...
//extern Student_Data receive_student_data;
...
extern void tuxin_task(void const *pvParameters);
#endif
